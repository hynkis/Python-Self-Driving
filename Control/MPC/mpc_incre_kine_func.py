...
def reference_search(path_x, path_y, path_yaw, set_speed, pred_state, dt, N):
    """
...
def simulate(init_state, path_x, path_y, path_yaw):
    """
    Inputs:
        - init_state : (X, Y, V, Yaw). numpy array [2x1] (ex. [[0],[0],[10],[0]])
        - path_x     [list]
        - path_y     [list]
        - path_yaw   [list]
    ...
    # ========== Objective function ==========
    # MPC weight matrix
    Q = sparse.diags([100.0, 100.0, 10.0, 100.0])         # weight matrix for state
    # QN = Q
    ...
    # States  : [x; y; v; yaw]
    # Actions : [steer; accel]
    x0 = init_state    #  [X; Y; V; Yaw]. (2,1) shape.
    ...
    set_speed = x0[2,0] # velocity when lane changing
    ...
    traj_yaw = []
    traj_steer = []
    traj_x.append(x0[0,0])
    traj_y.append(x0[1,0])
    traj_yaw.append(x0[3,0])
    traj_steer.append(u0[1,0])
    finish_cnt = 0 # count for finising simulation
    ...
    for i in range(sim_time):
        ...
        toc_mpc = time.time()
        print("mpc time :", toc_mpc-tic_mpc)
        ...
        dist = np.sqrt((Xr[0,0] - x_tilda[0,0])**2 + (Xr[1,0] - x_tilda[1,0])**2)
        print("dist :", dist)
        if dist < 0.5:
    ...
    return traj_x, traj_y, traj_yaw, traj_steer

def main():
    init_state = np.array([[0.0],[0.0],[10.0],[np.deg2rad(0)]])   # 2x1 shape numpy matrix
    path_x = np.linspace(-10, 100, 100/0.5)                       # list
    path_y = np.linspace(-10, 100, 100/0.5) * 0.0 - 4.0           # list
    path_yaw = np.linspace(-10, 100, 100/0.5) * 0.0               # list

    traj_x, traj_y, traj_yaw, traj_steer = simulate(init_state, path_x, path_y, path_yaw)
